...
import { beforeAll, beforeEach, bench, describe, test } from "vitest"
...
const generateTestObject = (depth: number, breadth: number): Record<string, unknown> => {
	if (depth <= 0) {
	...
	for (let i = 0; i < breadth; i++) {
		...
		if (i % 4 === 0) {
			...
			// Array of objects
			obj[key] = Array.from({ length: Math.max(1, Math.floor(breadth / 2)) }, (_) =>
				generateTestObject(Math.max(0, depth - 2), Math.max(1, Math.floor(breadth / 2))),
			)
		} else {
			// Primitive value
			obj[key] = `value${i}`
...
describe("Memory Impact Tests", () => {
	...
	test("structure-id memory impact", () => {
		...
		console.log(`Memory impact (structure-id): ${(endMemory - startMemory).toFixed(2)} MB`)
	})
	...
	test("sha256 memory impact", () => {
		...
		const endMemory = getMemoryUsage()
		console.log(`Ending memory: ${endMemory.toFixed(2)} MB`)
		console.log(`Memory impact (sha256): ${(endMemory - startMemory).toFixed(2)} MB`)
	})
})
