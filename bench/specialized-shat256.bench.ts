...
import { beforeEach, bench, describe, test } from "vitest"
...
// Test case generator
const createTestCase = (props: number, depth: number, arraySize = 0): Record<string, unknown> => {
	...
	for (let i = 0; i < props; i++) {
		if (depth > 1 && i % 3 === 0) {
			// Add nested object
			result[`nested${i}`] = createTestCase(Math.max(1, props - 1), depth - 1, arraySize)
